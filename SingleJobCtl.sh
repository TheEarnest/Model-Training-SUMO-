...
# run script 
  ...
  PmMeCoef=`echo ${Param} | awk -F " " '{print $4}' `
    ...
    ${fpath}/renewCOSMOSrun ${RSSExpid} ${RSSyear} ${expid} ${RSyear}
		cd ${exphome}/scripts
 		. ${fpath}/buildjobscript.sh
...
# post process
  ...
  if [ ! -f ${home}/Jobs/${ExpID}_PostCPI_* ] && [ "${JobR}"==" "  ]; then
    ${fpath}/SinglePostCPI ${expid} ${JobPeriod}  &> ${home}/log/odata_postCPI_${expid}_`date +%Y%m%d%H%M%S`
